 ...
 *
 * Fix applied: getAuth() now returns the user object (not void/Response).
 * All handlers use the returned user.id instead of session.user.id.
 *
 * Root cause of "Cannot find name 'user'" errors:
 * The previous getAuth() called getUser() internally but returned
 * NextResponse instead of the user — so `user` was never in scope
 * in the calling handlers. Fixed by making getAuth() return the
 * user object directly (or null on failure).
 */
...
/* -------------------------------------------------------------------------- */
/* Auth helper — returns user or null (callers handle the 401)               */
/* -------------------------------------------------------------------------- */

// FIX: returns the user object so callers have user.id in scope
...
async function uploadMediaToWA(file: File, token: string, phoneNumberId: string): Promise<string | null> {
  try {
...
function buildComponents(f: {
  template_type?: string; header_type?: string; header_text?: string
  header_handle?: string; header_examples?: string[]
  body_text: string; body_examples?: string[]; footer_text?: string
  buttons?: any[]
  add_security_recommendation?: boolean; code_expiration_minutes?: number
}) {
  const comps: any[] = []
  ...
  if (ht !== 'NONE' && ht !== '') {
    if (f.template_type !== 'AUTHENTICATION') {
      if (ht === 'TEXT') {
        ...
        if (f.header_examples?.length) hComp.example = { header_text: f.header_examples }
        comps.push(hComp)
      } else if (ht === 'LOCATION') {
        comps.push({ type: 'HEADER', format: 'LOCATION' })
      } else if (['IMAGE', 'VIDEO', 'DOCUMENT'].includes(ht)) {
        const hComp: any = { type: 'HEADER', format: ht }
        if (f.header_handle) hComp.example = { header_handle: [f.header_handle] }
        comps.push(hComp)
      }
    }
  }
  ...
  if (f.template_type === 'AUTHENTICATION') {
    ...
    if (examples.length > 0) bodyComp.example = { body_text: [examples] }
    comps.push(bodyComp)
  }
  ...
  if (btns.length > 0) {
    const mapped = btns.map((b: any) => {
      switch (b.type) {
        ...
        case 'FLOW': {
          // FIX: proper Meta API structure for FLOW button
          const flowBtn: any = {
            type:        'FLOW',
            text:        b.text || 'Open',
            flow_action: b.flow_action ?? 'navigate',
          }
          if (b.flow_id)         flowBtn.flow_id         = String(b.flow_id)
          if (b.navigate_screen) flowBtn.navigate_screen = b.navigate_screen
          return flowBtn
        }
        default: return b
      }
...
export async function GET(req: NextRequest) {
  // FIX: getAuthUser() returns the user object
  ...
  if (!doSync || !hasWaba) {
    const { data } = await admin.from('templates').select('*').eq('workspace_id', wsId).order('created_at', { ascending: false })
    return NextResponse.json({ templates: data ?? [], source: 'local', has_waba: hasWaba, missing: !wabaId ? 'WABA_ID' : !token ? 'TOKEN' : null })
  }
  ...
  try {
    ...
    while (nextUrl) {
      const res: any = await axios.get(nextUrl, { params, headers: { Authorization: `Bearer ${token}` } })
    ...
    for (const t of metaAll) {
      ...
      const { data: existing } = await admin.from('templates').select('id, meta').eq('workspace_id', wsId).eq('meta_template_id', String(t.id)).maybeSingle()
      if (existing) {
        if (existing.meta?.header_media_url) record.meta.header_media_url = existing.meta.header_media_url
    ...
    if (metaAll.length > 0) {
      ...
      const { data: localSynced } = await admin.from('templates').select('id, meta_template_id').eq('workspace_id', wsId).not('meta_template_id', 'is', null)
      if (localSynced) {
        const staleIds = localSynced.filter(r => !metaIds.includes(String(r.meta_template_id))).map(r => r.id)
        if (staleIds.length > 0) await admin.from('templates').delete().in('id', staleIds)
      }
    }
  } catch (err: any) {
  ...
  const { data: all } = await admin.from('templates').select('*').eq('workspace_id', wsId).order('created_at', { ascending: false })
  return NextResponse.json({ templates: all ?? [], meta_count: metaAll.length, source: 'meta_synced', has_waba: hasWaba, sync_error: syncError })
}
...
export async function POST(req: NextRequest) {
  ...
  const metaPayload = { name, category: f.category.toUpperCase(), language: f.language, components }
  ...
  if (wabaId && token) {
    try {
      const res = await axios.post(`${WA_BASE}/${wabaId}/message_templates`, metaPayload, { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' } })
...
export async function PATCH(req: NextRequest) {
  ...
  if (current.meta_template_id && token) {
    try {
      await axios.post(
        ...
        { name: finalName, components, language: finalLanguage, category: finalCategory.toUpperCase() },
  ...
  const { data: updated, error: dbErr } = await admin.from('templates').update({
    name:             finalName,
...
export async function DELETE(req: NextRequest) {
  ...
  if (wabaId && token && template_name) {
    try {
      ...
      await axios.delete(`${WA_BASE}/${wabaId}/message_templates`, { params, headers: { Authorization: `Bearer ${token}` } })
      metaDeleted = true
    } catch (err: any) {
      metaError = err?.response?.data?.error?.message ?? err.message
